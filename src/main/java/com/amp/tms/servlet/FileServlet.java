...
import com.amp.tms.service.GCESignedUtility;
...
public class FileServlet extends HttpServlet {
    ...
    private DialplanBuilderAutowireOb autowireOb;
    // Properties ---------------------------------------------------------------------------------
...
//            return;
            try {
                if (autowireOb == null) {
                    autowireOb = (DialplanBuilderAutowireOb) ApplicationContextProvider.getApplicationContext().getBean(DialplanBuilderAutowireOb.class);
                }
                GCESignedUtility gcssurl = autowireOb.getGcssurl();
                if(requestedFile.startsWith("/")){
                    requestedFile = requestedFile.substring(1);
                }
                log.info("requestedFile: " + requestedFile);    
                String url = "";
                if(basePath.contains("recordings"))
                       url = gcssurl.GetCallRecordingURL(requestedFile);
                else 
                       url = gcssurl.GetCallScreenRecordingURL(requestedFile);
