 ...
 * 
 */
public class IncomingPlaceOffHoldTransffer extends DialplanBuilder {

    private InboundDialerInfoPojo dialerInfoPojo;

    public IncomingPlaceOffHoldTransffer(InboundDialerInfoPojo dialerInfoPojo) {
        super();
    ...
    public void buildDialplans() {
        TMSDialplan old = dialplanService.findTMSDialplan(TMS_UUID, FreeswitchContext.fifo_dp, HOLDOrder.PLACE_ON_HOLD_TRANSFER.name());
        if (old == null) {
            old = dialplanService.findTMSDialplan(TMS_UUID, FreeswitchContext.fifo_dp, HOLDOrder.PLACE_ON_HOLD_CUSTOMER_SERVICE.name());
        }
        if (old == null) {
            old = dialplanService.findTMSDialplan(TMS_UUID, FreeswitchContext.fifo_dp, HOLDOrder.PLACE_ON_HOLD.name());
        }
        if (old == null) {
            for (int i = 0; i < 100; i++) {
        ...
        callOriginatingToFifo(old);
        callEnteringAgent(old);
        putBackIntoFifo();
        biuldVoicemailOption(old);
    ...
    public void commonVariable(TMSDialplan tMSDialplan, TMSDialplan old) {
        ...
        if (old != null) {
            if (old.getUniqueID() != null) {
                ...
                tMSDialplan.addAction(new Set("fifo_bridge_uuid=" + old.getFifoUniqueID()));
            }
        }
    }
